...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_EMPRESA" 
return int	
is
...
SELECT  e.CNPJ_BASICO                                                    AS root_cnpj 
	       ...
	       ,CASE WHEN s.OPC_SIMPLES = '' THEN 'OUTROS' 
	             ...
	             WHEN s.OPC_SIMPLES = 'N' THEN 'NÃO'  ELSE 'OUTROS' END AS opc_simples 
	       ,CASE WHEN s.OPC_MEI = '' THEN 'OUTROS' 
	             WHEN s.OPC_MEI = 'S' THEN 'SIM' 
	             WHEN s.OPC_MEI = 'N' THEN 'NÃO'  ELSE 'OUTROS' END     AS opc_mei 
	       ,q.id_uri                                                         AS id_qualif 
	       ,n.id_uri                                                         AS id_nat 
	       ,e.CAPITAL_SOCIAL                                                 AS capital_social 
	       ,CASE WHEN e.PORTE = '1' OR e.PORTE = '01' THEN 'NÃO INFORMADO' 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_EMPRESA_HOLDING" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_ENDERECO" 
return int	
is
    povoamento integer := 13;
begin
    ...
    SELECT   CASE WHEN e.CEP = '' THEN NULL ELSE cep END                                                                                   AS cep 
	       ...
	       ,CONCAT(e.CNPJ,e.CEP) AS id_endereco --ora_hashï¼¿nm_cidade_exterior || cod_pais||nome_pais||tipo_logradouro||logradouro||numero||complemento||bairro||cep||uf||cod_municipio||municipioï¼¿ AS id_endereco
        FROM
        (
           SELECT CEP, LOGRADOURO, BAIRRO, NUMERO, UF, COMPLEMENTO, COD_PAIS, TIPO_LOGRADOURO, COD_MUNICIPIO, CONCAT(CNPJ_BASICO,CONCAT(CNPJ_ORDEM,CNPJ_DV)) AS CNPJ FROM EXTR_ESTABELECIMENTO 
        ...
        begin
            insert into rfb_endereco values(ender.uri_endereco,ender.uf,ender.municipio,ender.logradouro,ender.numero,ender.complemento,ender.cep,ender.bairro,ender.nome_pais, ender.id_endereco);
            commit;
    --end loop;
            exception
...
begin
    ...
    (
        SELECT  DISTINCT CASE WHEN e.CEP = '' THEN NULL ELSE cep END                                                                                   AS cep 
	       ...
	       ,CASE WHEN e.NUMERO = '' THEN NULL  ELSE numero END                                                                                     AS numero 
	       ,CASE WHEN e.COMPLEMENTO = '' THEN NULL  ELSE complemento END                                                                           AS complemento 
	       ,dbms_crypto.Hash(utl_raw.cast_to_raw(e.COD_PAIS ||p.NOME_PAIS||e.TIPO_LOGRADOURO||e.LOGRADOURO||e.NUMERO||e.COMPLEMENTO||e.BAIRRO||e.CEP||e.UF||e.COD_MUNICIPIO||m.MUNICIPIO),2) AS id_endereco --ora_hashï¼¿nm_cidade_exterior || cod_pais||nome_pais||tipo_logradouro||logradouro||numero||complemento||bairro||cep||uf||cod_municipio||municipioï¼¿ AS id_endereco
        FROM
        (
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_ENDERECO_EXTERIOR" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_ESTABELECIMENTO" 
return int	
is
    povoamento integer := 13;
begin
...
SELECT -- 'http://www.sefaz.ma.gov.br/resource/RFB/Estabelecimento/'||CONCAT(CONCAT(cnpj,'-'),UF) as uri -- uri OK 
       ...
       ,'http://www.sefaz.ma.gov.br/resource/RFB/Estabelecimento/'||cnpj as uri -- uri OK 
       ,cnae_fiscal 
       ,id_endereco
       ,situacao
FROM 
(
	SELECT  /*DISTINCT*/ CONCAT(est.CNPJ_BASICO,CONCAT(est.CNPJ_ORDEM,est.CNPJ_DV))                                AS cnpj 
         ...
         ,CASE est.NOME_FANTASIA WHEN '' THEN NULL ELSE est.NOME_FANTASIA END                                                                               AS nome_fantasia 
	       ...
	       ,est.CNAE_FISCAL 
	       ,CONCAT(CONCAT(CONCAT(est.CNPJ_BASICO,est.CNPJ_ORDEM),est.CNPJ_DV),est.CEP) AS id_endereco 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_ESTABELECIMENTO_EXTERIOR" 
return int	
is
    povoamento integer := 13;
begin
...
SELECT 'http://www.sefaz.ma.gov.br/resource/RFB/Estabelecimento/'||CONCAT(CONCAT(cnpj,'-'),UF) as uri
       ...
       ,data_inicio_ativ 
			 --,null -- uri_endereço_nacional ok 
       ,concat('http://www.sefaz.ma.gov.br/resource/RFB/Endereco/',id_endereco) -- uri_endereço_exterior ok 
       ,phone_1 
       ,phone_2  
       ,fax 
       ,CONCAT('http://www.sefaz.ma.gov.br/resource/RFB/Situacao_Cadastral/',CONCAT(CONCAT(CONCAT(CONCAT(situacao,'-'),cnpj),'-'),data_situacao)) 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_ESTRANGEIRO" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_FAIXA_ETARIA" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
INSERT ALL
   ...
   INTO RFB_FAIXA_ETARIA (uri_faixa_etaria, codigo, descricao) VALUES (0, 0, 'Não se aplica')
SELECT 1 FROM DUAL;
return povoamento;
end;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_NATUREZA_LEGAL" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_OPCAO_MEI" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_OPCAO_SIMPLES" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_PESSOA" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
begin
    ...
    SELECT  'http://www.sefaz.ma.gov.br/resource/RFB/Pessoa/'||CONCAT(CONCAT(max(uri_cpf),'-'),max(uri_nome)) -- uri ok 
       ...
       ,nome_socio -- nome ok 
       ,max(faixa_etaria) -- faixa ok OBS: NÃ¿O CONSIGO PUXAR A FAIXA ETÃ?RIA DO REPRESENTANTE FICARÃ? COMO 0 NÃ¿O SE APLICA
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_QUALIFICACAO_SOCIO" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_RAZAO_SITUACAO_CADASTRAL" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_SITUACAO_CADASTRAL" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_SITUACAO_ESPECIAL" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_SOCIEDADE_COM_ESTRANGEIRO" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_SOCIEDADE_COM_HOLDING" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_SOCIEDADE_COM_PESSOA_FISICA" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_TEM_ATIV_ECONOMICA_SECUNDARIA" 
return int	
is
...
begin
    ...
    loop
        ...
        if cnae is not null then
            insert into RFB_TEM_ATIV_ECONOMICA_SECUNDARIA VALUES ('http://www.sefaz.ma.gov.br/resource/RFB/Estabelecimento/'||concat(CNPJ_BASICO,concat(CNPJ_ORDEM, CNPJ_DV)), 'http://www.sefaz.ma.gov.br/resource/RFB/Atividade_Economica/'||SUBSTR(cnae,0,7), concat(CNPJ_BASICO,concat(CNPJ_ORDEM, concat(CNPJ_BASICO,concat(CNPJ_ORDEM, CNPJ_DV)))), SUBSTR(cnae,0,7));
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_TEM_ESTABELECIMENTO" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_TEM_SITUACAO_ESPECIAL" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_TEM_SOCIEDADE_ESTRANGEIRA" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
SELECT  CONCAT('http://www.sefaz.ma.gov.br/resource/RFB/Empresa/',root_cnpj) 
       ,CONCAT('http://www.sefaz.ma.gov.br/resource/RFB/Sociedade/',(id_uri||'-'||root_cnpj||'-'||uri_socio))
       ,root_cnpj AS id_empresa
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_TEM_SOCIEDADE_FISICA" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE FUNCTION "POVOA_RFB_TEM_SOCIEDADE_JURIDICA" 
return int	
is
	--declaracao das variaveis emp
    povoamento integer := 13;
