...
SELECT  CONCAT(CONCAT(cnpj,'-'),UF) as uri
       ,cnpj  
       ,razao_social
       ,nome_fantasia 
       ,email 
       ...
       ,phone_2 
       ,fax 
       ,CONCAT(CONCAT(CONCAT(CONCAT(situacao,'-'),cnpj),'-'),data_situacao) 
FROM 
(
...
SELECT CONCAT(CONCAT(cnpj,'-'),UF) 
       ...
       ,id_endereco -- uri_endereÁo_exterior ok 
       ,phone_1 
       ,phone_2  
       ,fax 
       ,CONCAT(CONCAT(CONCAT(CONCAT(situacao,'-'),cnpj),'-'),data_situacao) 
...
(
		SELECT  /*DISTINCT*/ CONCAT(est.CNPJ_BASICO,CONCAT(est.CNPJ_ORDEM,est.CNPJ_DV))                                AS cnpj
	       ...
	       ,CASE est.MATRIZ_FILIAL WHEN '1' THEN 'MATRIZ' ELSE 'FILIAL' END                                                                   AS matriz_filial 
	       ,est.CNAE_FISCAL 
	       ,dbms_crypto.Hash(utl_raw.cast_to_raw(est.NM_CIDADE_EXTERIOR ||est.COD_PAIS ||p.NOME_PAIS||est.CEP),2)                           AS id_endereco 
...
SELECT  CONCAT(CONCAT(OPC_MEI ,'-'),root_cnpj) 
       ,tipo_mei 
       ,data_exc_mei 
       ,data_opc_mei 
FROM 
...
SELECT  CONCAT(CONCAT(opc_simples,'-'),root_cnpj) 
       ,simples_type 
       ,class_simples
       ,data_exc_simples 
       ,data_opc_simples
...
begin
    ...
    SELECT  CONCAT(CONCAT(max(uri_cpf),'-'),max(uri_nome)) -- uri ok 
...
SELECT id_uri,Descricao,cast(Codigo as int) FROM (
...
SELECT (situacao||'-'||cnpj||'-'||data_situacao),val_data_situacao,type_situacao,id_uri  FROM (
SELECT DISTINCT CONCAT(CNPJ_BASICO,CONCAT(CNPJ_ORDEM,CNPJ_DV)) as cnpj,
  REPLACE(regexp_replace(nome_fantasia, '[^a-zA-Z ]', ''),' ','_') as nome_uri,
				...
				CASE situacao WHEN '1' THEN 'NULA'
					WHEN '2' THEN 'ATIVA'
...
SELECT  (root_cnpj||'-'||data_sit_especial) 
       ,data_sit  
       ,sit_especial
FROM 
(
...
SELECT  (id_uri||'-'||root_cnpj||'-'||uri_socio) AS uri_ 
       ,MAX(data_entrada)
       ,MAX(descricao) 
       ,MAX(rep_quali) 
       ,MAX(uri_socio) 
       ,MAX(CONCAT(CONCAT(cpf_rep,'-'),nome_rep)) 
       ,MAX(id_uri) 
FROM 
(
...
begin
    ...
    SELECT  (id_uri||'-'||root_cnpj||'-'||cnpj_cpf_socio||'-'||nome_socio) AS uri_ 
       ,MAX(descricao)
...
begin
    ...
    SELECT  (id_uri||'-'||root_cnpj||'-'||cnpj_cpf_socio||'-'||nome_socio) AS uri_ 
       ...
       ,MAX(CONCAT(CONCAT(cnpj_cpf_socio,'-'),nome_socio)) 
       ,MAX(id_uri)
       ,MAX(CONCAT(CONCAT(cnpj_cpf_representante,'-'),nome_representante)) 
       ,MAX(id_uri_representante)
    FROM 
    (
...
begin
    ...
    loop
        ...
        if cnae is not null then
            insert into RFB_TEM_ATIV_ECONOMICA_SECUNDARIA VALUES (concat(CNPJ_BASICO,concat(CNPJ_ORDEM, concat(CNPJ_DV, concat('-',UF)))), SUBSTR(cnae,0,7));
            ...
            while indice_f > 0 loop
                ...
                insert into RFB_TEM_ATIV_ECONOMICA_SECUNDARIA VALUES (concat(CNPJ_BASICO,concat(CNPJ_ORDEM, concat(CNPJ_DV, concat('-',UF)))), substr(cnae, indice_i+8, 7));
                indice_i := indice_f +1;
...
SELECT  CONCAT(CONCAT(root_cnpj_e,'-'),razao_uri_e) 
       ,CONCAT(CONCAT(cnpj_f,'-'),UF)
FROM 
(
...
SELECT  CONCAT(CONCAT(root_cnpj,'-'),razao_uri)  
       ,CONCAT(CONCAT(root_cnpj,'-'),data_sit_especial)
FROM 
(
...
SELECT  CONCAT(CONCAT(root_cnpj,'-'),razao_uri) 
       ,(id_uri||'-'||root_cnpj||'-'||cnpj_cpf_socio||'-'||nome_socio)
FROM 
(
	SELECT  DISTINCT s.CNPJ_BASICO                                        AS root_cnpj 
